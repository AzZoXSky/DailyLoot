...
class DailyLootModel extends AbstractModel
{
    ...
    /**
     * Met à jour une récompense dans la base de données.
     */
    public function updateReward(int $id, string $name, string $rarity, float $probability, string $command, ?string $image = null): void
    {
        $sql = "UPDATE " . self::$tableRewards . " 
            SET name = :name, rarity = :rarity, probability = :probability, command = :command, image = :image 
            WHERE id = :id";
        $db = DatabaseManager::getInstance();
        $req = $db->prepare($sql);
