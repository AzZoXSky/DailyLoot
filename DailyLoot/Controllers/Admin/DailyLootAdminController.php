...
class DailyLootAdminController extends AbstractController
{
    ...
    #[Link('/manage/edit', Link::POST, [], '/cmw-admin/dailyloot')]
    private function dailyLootEditPost(): void
    {
        UsersController::redirectIfNotHavePermissions('core.dashboard', 'dailyloot.edit');

        $dailyLootModel = DailyLootModel::getInstance();
        [$rewardId, $rewardName, $rarity, $probability, $command, $image] = Utils::filterInput('reward_id', 'reward_name', 'rarity', 'probability', 'command', 'image');

        $message = '';
        $isError = false;

        if (!$rewardId || !$rewardName || !$rarity || !$probability || !$command || !$image) {
            $message = "Tous les champs sont requis.";
            $isError = true;
        } else {
            $dailyLootModel->updateReward($rewardId, $rewardName, $rarity, $probability, $command, $image);
            Flash::send(Alert::SUCCESS, 'DailyLoot', 'La récompense a été modifiée avec succès.');
        }
